...
if (grepl("experiment1", expt_dir)) {
  ...
  gnn_conc_pth <- paste0(expt_dir,
                         ...
                         "feature-selection=no-sagpool_concordance.csv")
  ...
  gnn_conc_fs_pth <- paste0(expt_dir,
                            "gnn_reactome_unmerged_directed_nonrelational_",
                            "feature-selection=sagpool_concordance.csv")
  ...
  file_out <- paste0(
    expt_dir, "concordance_unpaired_two-sided_welch_pvals.csv"
  )
  ...
  row.names(res) <- c("no-sagpool", "sagpool")
  file_out <- paste0(
    expt_dir, "concordance_unpaired_gnn-greater_welch_pvals.csv"
  )
  write.csv(res, file_out)
  ...
  row.names(res) <- c("no-sagpool", "sagpool")
  file_out <- paste0(
    expt_dir, "concordance_unpaired_gnn-less_welch_pvals.csv"
  )
  write.csv(res, file_out)
  
  # Test whether the concordance scores of models trained in different cancer
  ...
  file_out <- paste0(
    expt_dir, "concordance_gnn_mlp_feature-selection_paired_wilcoxon_fdr.csv"
  )
  ...
  # mean GNN concordance for a cancer type is less than the MLP's
  ...
  # mean GNN concordance for a cancer type is greater than the MLP's

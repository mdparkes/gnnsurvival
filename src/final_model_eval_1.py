"""
Evaluate performance of models from experiment 1
"""
...
if __name__ == "__main__":
    ...
    int_brier_dict = dict(zip(model_types, [dict(), dict(), dict(), dict()]))
    int_brier_means_dict = dict(zip(model_types, [dict(), dict(), dict(), dict()]))
    int_brier_sem_dict = dict(zip(model_types, [dict(), dict(), dict(), dict()]))
    L1_loss_dict = dict(zip(model_types, [dict(), dict(), dict(), dict()]))
    L1_loss_means_dict = dict(zip(model_types, [dict(), dict(), dict(), dict()]))
    L1_loss_sem_dict = dict(zip(model_types, [dict(), dict(), dict(), dict()]))
    d_cal_dict = dict(zip(model_types, [dict(), dict(), dict(), dict()]))
    ...
    file_name = os.path.join(output_dir, f"{gnn_shared_segment}no-sagpool_concordance.csv")
    concordance["GNN - No SAGPool"].to_csv(file_name)
    ...
    file_name = os.path.join(output_dir, f"{gnn_shared_segment}no-sagpool_int_brier.csv")
    int_brier["GNN - No SAGPool"].to_csv(file_name)
    ...
    file_name = os.path.join(output_dir, f"{gnn_shared_segment}no-sagpool_L1_loss.csv")
    L1_loss["GNN - No SAGPool"].to_csv(file_name)
    ...
    file_name = os.path.join(output_dir, f"{gnn_shared_segment}no-sagpool_d_cal.csv")
    d_cal["GNN - No SAGPool"].to_csv(file_name)
    ...
    fig.legend(loc="outside upper center", labels=model_types, ncols=2)
